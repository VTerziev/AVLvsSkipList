...
bool AVLTree::contains(int x) { return findNode(root, x) != nullptr; }

void AVLTree::insert(int x) { root = addNode(root, x); }

void AVLTree::remove(int x) { root = removeValue(root, x, 1); }

void AVLTree::print() { printTree(root); }
...
AvlNode *AVLTree::addNode(AvlNode *root, int value) {
  if (root == nullptr) {
    ...
    AvlNode *crr = Allocator::allocate<AvlNode>();
    crr->value = value;
    return crr;
